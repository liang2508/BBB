...
for i in range(1,11):
    ...
    #先搜索最优参数
    params = {'n_estimators': range(10, 510, 30), 'max_depth': range(1, 61, 5), 'class_weight': [{0: weight, 1: 1}]}
    estimator = GridSearchCV(ExtraTreesClassifier(random_state=0), params, cv=5, refit=True, n_jobs=12,
                             scoring=make_scorer(cohen_kappa_score))
    estimator.fit(X_train, y_train)
    best_param = estimator.best_params_
    clf = ExtraTreesClassifier(n_estimators=best_param['n_estimators'], max_depth=best_param['max_depth'], min_samples_split=3, min_samples_leaf=1, random_state=7,
                               max_features='auto',n_jobs=12, class_weight={0: weight, 1: 1})
    ...
    ##输出模型
    with open('./result/ERF_rdkit2d_{0}.pkl'.format(i), 'wb') as f:
        pickle.dump(clf, f)
        f.close()
...
statistics.to_csv('./result/ERF_statistics_rdkit2d.csv',header=True)
...
for i in range(1,11):
    ...
    if i == 1:
        ...
        statistics = pd.DataFrame(np.vstack((statistics, statistics_test)))
    ##输出模型
    with open('./result/ERF_rdkit3d_{0}.pkl'.format(i), 'wb') as f:
        pickle.dump(clf, f)
        f.close()

statistics.columns = ['AUC','BAC','F1','MCC','kappa','acc','precision','recall','sensitivity','specificity']
statistics.index=['Test1','Test2','Test3','Test4','Test5','Test6','Test7','Test8','Test9','Test10']
statistics.to_csv('./result/ERF_statistics_rdkit3d.csv',header=True)
...
for i in range(1,11):
    ...
    if i == 1:
        ...
        statistics = pd.DataFrame(np.vstack((statistics, statistics_test)))
    ##输出模型
    with open('./result/ERF_ecfp4_{0}.pkl'.format(i), 'wb') as f:
        pickle.dump(clf, f)
        f.close()

statistics.columns = ['AUC','BAC','F1','MCC','kappa','acc','precision','recall','sensitivity','specificity']
statistics.index=['Test1','Test2','Test3','Test4','Test5','Test6','Test7','Test8','Test9','Test10']
statistics.to_csv('./result/ERF_statistics_ecfp4.csv',header=True)
...
for i in range(1,11):
    ...
    if i == 1:
        ...
        statistics = pd.DataFrame(np.vstack((statistics, statistics_test)))
    ##输出模型
    with open('./result/ERF_all_{0}.pkl'.format(i), 'wb') as f:
        pickle.dump(clf, f)
        f.close()

statistics.columns = ['AUC','BAC','F1','MCC','kappa','acc','precision','recall','sensitivity','specificity']
statistics.index=['Test1','Test2','Test3','Test4','Test5','Test6','Test7','Test8','Test9','Test10']
statistics.to_csv('./result/ERF_statistics_all.csv',header=True)